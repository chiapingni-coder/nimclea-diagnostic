...
| `docs/CASE_DELETE_DISCARD_BACKEND_CONTRACT.md` | Backend delete / discard contract for ordinary unpaid active cases. Defines irreversible deletion scope, minimal non-recoverable denylist rules, payment-pending high-risk deletion, Baseline/Historic no-delete rules, `/cases` filtering requirements, and frontend local delete boundary. |
...
| `docs/CASE_DELETE_RETENTION_CONTRACT.md` | Delete contract for ordinary unpaid cases, unpaid pending checkout cases, and protected permanent records. Defines irreversible ordinary-case deletion, minimal non-recoverable denylist records, and frontend/backend delete eligibility. |
| `docs/CASE_DELETE_RETENTION_SMOKE_CHECKPOINT.md` | Historical Delete / Retention smoke checkpoint. Records that older recoverable-retention language is superseded by irreversible ordinary-case deletion and denylist-only rebuild prevention. |
| `docs/CASE_LIFECYCLE_TABS_UI_SMOKE_CHECKPOINT.md` | Lifecycle tabs UI smoke checkpoint for Active / Baseline / Historic case grouping on the Cases page. |
